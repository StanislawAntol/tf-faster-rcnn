...
## Freeze Region-of-Interest (MobileNet) Model as .pb File
In order to freeze`.pb` version of the Region-of-Interest (RoI) model (i.e., no classification/bounding box refinement), you can do follow the following steps.

* `git clone https://github.com/StanislawAntol/tf-faster-rcnn.git tf-faster-rcnn-stan`
* `git checkout freeze_model`
* `cd tf-faster-rcnn-stan/lib`
* `make clean`
* `make`
* `cd ../tools/`
* `python freeze_as_pb.py`

`lib/model/config.py` has some parameters (`__C.TEST.RPN_NMS_THRESH` and `__C.TEST.RPN_POST_NMS_TOP_N`) to tweak to change how many region proposals are generated by the network.

By default, it assumes the MobileNet model has been downloaded and extracted to `/tmp/models/coco_2014_train+coco_2014_valminusminival`, which is where it will also save the `.pb` file to. 

`python freeze_as_pb.py --help` provides an explanation to the arguments to change input path, output path, which output nodes to use, and whether or not to print all the node names.

Currently, the model gets frozen with pre-processing embedded in the graph (input, `Placeholder:0`, is an RGB UINT8, which gets normalized and resized such that the shortest side is 600 or the longest side is 1000). The output (`MobilenetV1_2/rois/concat_1:0`) is `__C.TEST.RPN_POST_NMS_TOP_` * 5, where first element is batch index (ignore for single image), then `x1`, `y1`, `x2`, `y2`, for each RoI.
